...
<?php 
  // untuk mendambah form & menyimpan inputan cekno ttp keliatan
    ...
    if(isset($_POST['submit']) && $_POST['submit'] == 'SIMPAN'){
      include 'upload.php';
      include 'form_mahasiswa_insert.php';
    }
?>
<div style="width:700px;">
<form name="form" method="post" action="form_mahasiswa.php" enctype="multipart/form-data">
<input type="hidden" id="angka_kredit"  name="angka_kredit"  value="<?php echo $_GET['id'];?>" />
  ...
  <?php for ($i=0; $i < $form; $i++) { ?>
    <tr>
      <td colspan="3"><h3>Mahasiswa <?php echo $i+1; ?></h3> </td>
    </tr>
    <tr class="form-group">
    <!-- inputan -->
      <td>Nama</td>
    ...
    <tr class="form-group">
      <td>NIM</td>
      <td>:</td>
      <td><input type="text" name="nim[]" id="nim" class="form-control" value="<?php echo (empty($nim[$i])) ? '' : $nim[$i]; ?>"></td>
    </tr>
    <tr class="form-group">
