...
<?php 
  // untuk mendambah form & menyimpan inputan cekno ttp keliatan
    ...
    if(isset($_POST['submit']) && $_POST['submit'] == 'SIMPAN'){
      include 'upload.php';
      include 'form_sks_insert.php';
    }
?>
<div style="width:700px;">
<form name="form" method="post" action="form_sks.php" enctype="multipart/form-data">
<input type="hidden" id="angka_kredit"  name="angka_kredit"  value="<?php echo $_GET['id'];?>" />
  ...
  <?php for ($i=0; $i < $form; $i++) { ?>
    <tr>
      <td colspan="3"><h3>matakuliah <?php echo $i+1; ?></h3> </td>
    </tr>
    <tr class="form-group">
    <!-- inputan -->
      <td>Nama Mata Kuliah</td>
      <td width="5%">:</td>
      <td><input type="text" name="matakuliah[]" id="matakuliah" class="form-control" value="<?php echo (empty($matakuliah[$i])) ? '' : $matakuliah[$i]; ?>"></td>
    </tr>
    <tr class="form-group">
    ...
    <tr class="form-group">
      <td>SKS</td>
      <td>:</td>
      <td><input type="text" name="sks[]" id="sks" class="form-control" value="<?php echo (empty($sks[$i])) ? '' : $sks[$i]; ?>"></td>
    </tr>
    <tr class="form-group">
      <td>keterangan</td>
      <td>:</td>
