...
<?php 
    ...
    if(isset($_POST['submit']) && $_POST['submit'] == 'SIMPAN'){
      include 'upload.php';
      include 'form_@penunjang_insert.php';
...
<form name="form" method="post" action="form_@penunjang.php" enctype="multipart/form-data">
  <!--di copas -->
  ...
  <?php for ($i=0; $i < $form; $i++) { ?>
    <tr>
    	<td colspan="3"><h3>Penunjang Kegiatan <?php echo $i+1; ?></h3> </td>
    </tr>
    <tr class="form-group">
  	  <td>Nama Kegiatan</td>
  	  <td width="5%">:</td>
    ...
    <tr class="form-group">
      <td>Kedudukan/Tingkat</td>
      <td>:</td>
      <td><input type="text" name="kedudukan[]" id="kedudukan" class="form-control" value="<?php echo (empty($kedudukan[$i])) ? '' : $kedudukan[$i]; ?>"></td>
    </tr>
    <tr class="form-group">
