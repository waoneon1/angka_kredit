...
<?php 
    ...
    if(isset($_POST['submit']) && $_POST['submit'] == 'SIMPAN'){
      ...
      print_r($_FILES);exit;*/
      include 'upload.php';
      include 'form_sertifikat_insert.php';
    	// disini di buat save ke database
    	//kalo mau lihat isinya coba di var_dump($_POST);
...
<form name="form" method="post" action="form_sertifikat.php?id=<?php echo $_GET['id'];?>" enctype="multipart/form-data">
  ...
  <?php for ($i=0; $i < $form; $i++) { ?>
    <tr>
    	<td colspan="3"><h3>Sertifikat <?php echo $i+1; ?></h3> </td>
    </tr>
    <tr class="form-group">
  	  <td>Judul Sertifikat</td>
  	  <td width="5%">:</td>
  	  <td><input type="text" name="sertifikat[]" id="sertifikat" class="form-control" value="<?php echo (empty($sertifikat[$i])) ? '' : $sertifikat[$i]; ?>"></td>
    </tr>
    <tr class="form-group">
      <td>Tempat</td>
      <td>:</td>
